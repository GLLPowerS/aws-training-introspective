...
<sub> Creates a DaemonSet in kube-system that tails /var/log/containers and ships to:</sub><br>
<sub> /aws/containerinsights/cl-01/application</sub><br>
```
kubectl apply -k k8s/aws-logging
```

<sub> Verify it’s running </sub><br>
```
kubectl -n kube-system get pods -l k8s-app=aws-for-fluent-bit
```

## Dapr tracing to AWS X-Ray
<sub> Terraform creates IRSA role introspect-aws-otel-collector-irsa (attach AWSXRayDaemonWriteAccess) </sub><br>
```
